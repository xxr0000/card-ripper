...
async function playerPhotoFor(name: string): Promise<SportsDbPlayer | undefined> {
  ...
  return payload.player?.find((player) => player.strPlayer && acceptedNames.has(mediaKey(player.strPlayer)) && player.strThumb);
}
...
async function main(): Promise<void> {
  ...
  const corePlayers = PLAYERS.slice(offset, offset + limit);
  ...
  if (process.argv.includes('--finalize')) {
    ...
    const expected = PLAYERS.slice(0, 100).length;
    console.log(`高清图片试点完成 ${Object.keys(media).length}/${expected}；大图规格 ${PLAYER_LARGE_WIDTH}x${PLAYER_LARGE_HEIGHT}。`);
    if (Object.keys(media).length !== expected) process.exitCode = 1;
    return;
  }

  await Promise.all(corePlayers.map(async (player) => {
  ...
  console.log(`批次完成 ${Object.keys(media).length}/${corePlayers.length}。`);
  if (failures.length) {
